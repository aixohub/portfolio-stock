package com.aixohub.portfolio.util.impl;
...
public class ImageUtilSWTImpl extends ImageUtil {
    ...
    public String loadAndPrepare(String filename, int maxWidth, int maxHeight) throws IOException {
        ...
        ImageData imgData = toImageData(data);
        if (imgData == null)
            return null;

        if (imgData.width > maxWidth || imgData.height > maxHeight) {
            imgData = resize(imgData, maxWidth, maxHeight, true);
            data = encode(imgData);
        }
    ...
    private static class ZoomingImageDataProvider implements ImageDataProvider {
        ...
        private final ImageData fullSize;
        private final HashMap<Integer, ImageData> zoomLevels = new HashMap<>();

        public ZoomingImageDataProvider(byte[] data, int logicalWidth, int logicalHeight) {
            this.logicalWidth = logicalWidth;
            this.logicalHeight = logicalHeight;
            this.fullSize = toImageData(data);
        }
        ...
        public ImageData getImageData(int zoom) {
            ...
            imageData = resize(fullSize, (int) (fullSize.width * scaleW), (int) (fullSize.height * scaleH), false);
