...
import com.aixohub.portfolio.datatransfer.csv.CSVImporter.EnumField;
import com.aixohub.portfolio.datatransfer.csv.CSVImporter.Field;
import com.aixohub.portfolio.datatransfer.csv.CSVImporter.ISINField;
...
/* package */ class CSVAccountTransactionExtractor extends BaseCSVExtractor {
    /* package */ CSVAccountTransactionExtractor(Client client) {
        ...
        fields.add(new EnumField<Type>("type", Messages.CSVColumn_Type, Type.class) //$NON-NLS-1$
                .setOptional(true));
    ...
    private Type inferType(String[] rawValues, Map<String, Column> field2column, Security security, Money amount)
        ...
        if (type == null) {
            if (security != null)
                type = amount.isNegative() ? Type.REMOVAL : Type.DIVIDENDS;
            else
                type = amount.isNegative() ? Type.REMOVAL : Type.DEPOSIT;
        }
        return type;
    }
}
